    ...
    public boolean checkTextRight() {
        ...
        if (contentType == TYPE_PHONE) {
            if (TextUtils.isEmpty(text)) {
                Tips.toToast(getContext(), "手机号不能为空，请输入正确的手机号");
            } else if (text.length() < 11) {
                Tips.toToast(getContext(), "手机号不足11位，请输入正确的手机号");
            } else {
                return true;
            }
        } else if (contentType == TYPE_CARD) {
            if (TextUtils.isEmpty(text)) {
                Tips.toToast(getContext(), "银行卡号不能为空，请输入正确的银行卡号");
            } else if (text.length() < 14) {
                Tips.toToast(getContext(), "银行卡号位数不正确，请输入正确的银行卡号");
            } else {
                return true;
            }
        } else if (contentType == TYPE_IDCARD) {
            if (TextUtils.isEmpty(text)) {
                Tips.toToast(getContext(), "身份证号不能为空，请输入正确的身份证号");
            } else if (text.length() < 18) {
                Tips.toToast(getContext(), "身份证号不正确，请输入正确的身份证号");
            } else {
                return true;
            }
        }
